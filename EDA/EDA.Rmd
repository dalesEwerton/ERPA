...
institutos = data %>%
  group_by(Instituto) %>%
  summarise(vezes_apareceu = length(Instituto)) %>%
  arrange(desc(vezes_apareceu))

ggplot(data = institutos, 
       aes(x=reorder(Instituto, - as.numeric(vezes_apareceu)),
          y = as.factor(vezes_apareceu), fill = Instituto)) +
  geom_bar(stat = "identity", binwidth = NULL) +
...
anos = data %>%
  ...
  summarise(vezes_apareceu = length(Ano_da_Eleicao)) %>%
  arrange(desc(vezes_apareceu))

ggplot(data = anos, 
       aes(x=reorder(Ano_da_Eleicao, - as.numeric(vezes_apareceu)),
          y = as.factor(vezes_apareceu), fill = Ano_da_Eleicao)) +
  geom_bar(stat = "identity", binwidth = NULL) +
  labs(title = "Anos de eleição na Base de Dados",
       x = "Ano_da_Eleicao", y = "Vezes que aparece nos dados") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
